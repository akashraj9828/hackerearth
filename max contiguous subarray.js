...
// ******************** LOGIC ****************//
// O(n) solution. 
// start adding elemnts from starting and maintain a sum of them
// reset the best_till_now (best_sum) to element A[i] if sum(till A[i])<A[i]
// return the best of all best_till_now's
// *******************************************//
...
for (i = 0; i < A.length; i++) {
    best_till_now += A[i];
    if (best_till_now > 0)
        console.log("best: " + best_till_now + "    elelment: " + A[i]);

    if (best_till_now < A[i]) {
        //    console.log("---------------reset best to "+A[i] +" -------------------");
        best_till_now = A[i];
        start_pos = i;
    }
    if (best_till_now > max) {
        max = best_till_now;
    }
}
console.log(max);
